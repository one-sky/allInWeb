...
module.exports = function getEntriesRouter({ srcPath, distPath }, ignoreNeedPack) {
    const entries = {};
    let routers = [];
    const ROOT_PATH_LEN = path.resolve(__dirname, srcPath).length + 1;
        ...
        if (fs.statSync(dir).isDirectory()) {
            ...
            dirNames.forEach((item) => {
                ...
                if (STAT.isFile() && /\/View.js$/.test(item)) {
                    ...
                    FILE_CONTENT.replace(/YOOGA.allPage\[['"](.+)['"]\]/g, (matchStr, $0) => {
                        routers[$0] = `/yooga/${distPath}/${entryKey}.js`.replace(/\/+/g, '/');
                    });
